...
const RestaurantCard = ({ resData }) => {
  return (
    <div className="group flex flex-col rounded-lg overflow-hidden cursor-pointer h-72 bg-cyan-600 border border-cyan-700/30 transition-colors duration-200 hover:bg-cyan-700 hover:border-cyan-800/50">
      ...
      <div className="p-3 flex-1 flex flex-col group-hover:bg-cyan-800/20 transition-colors">
        <h3 className="m-0 text-base font-semibold text-white truncate">{resData?.info?.name || resData?.resName}</h3>

        <p className="m-0 text-sm text-gray-100 truncate mt-1">{resData?.info?.cuisines?.join(', ') || 'Beverages, juice'}</p>
        ...
        <div className="flex justify-between text-sm mt-auto items-center">
          ...
          <span className="text-amber-300 font-medium">{resData?.info?.sla?.deliveryTime || '30'} min</span>
          <span className="text-white font-semibold">{resData?.info?.costForTwo || '₹200'}</span>
        </div>
      </div>
    </div>
  );
